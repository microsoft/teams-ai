...
an abstraction that is meant to send or receive activities. To make migrating stress free, we have
shipped a pre-built `BotBuilderPlugin` that can accept a botbuilder Adapter instance.

:::info
This snippet shows how to use the `BotBuilderPlugin` to send or receive activities using
