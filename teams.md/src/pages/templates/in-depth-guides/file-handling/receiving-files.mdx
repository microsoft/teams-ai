...
<sup id="access-denied">1</sup> **Access denied** deliberately collapses causes the SDK cannot differentiate: an unconsented permission, a file the identity was never granted, and an item that does not exist all arrive as `403`. There is no separate "file not found" case to branch on; `details` carries the service's own text for diagnosis.
