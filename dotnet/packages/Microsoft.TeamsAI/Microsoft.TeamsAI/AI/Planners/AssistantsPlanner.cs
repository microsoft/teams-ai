...
#pragma warning restore IDE0130 // Namespace does not match folder structure
{
        ...
        where TState : TurnState, IAssistantsState
    {
        ...
        private readonly OpenAIAssistantsClient _client;
        ...
        public static async Task<Assistant> CreateAssistantAsync(string apiKey, AssistantCreateParams request, string? endpoint = null, string? organization = null, CancellationToken cancellationToken = default)
        {
            Verify.ParamNotNull(apiKey);
            Verify.ParamNotNull(request);

            OpenAIAssistantsClient client = new(new OpenAIAssistantsClientOptions(apiKey, endpoint)
            {
        ...
        public AssistantsPlanner(AssistantsPlannerOptions options, ILoggerFactory? loggerFactory = null)
        {
            ...
            _options = options;
            _options.PollingInterval ??= DEFAULT_POLLING_INTERVAL;

            OpenAIAssistantsClientOptions clientOptions = new(options.ApiKey, options.Endpoint)
            {
                Organization = options.Organization
            };

            _client = new OpenAIAssistantsClient(clientOptions);
        ...
        private async Task<string> _EnsureThreadCreatedAsync(TState state, CancellationToken cancellationToken)
        {
            ...
            {
                OpenAI.Models.Thread thread = await _client.CreateThreadAsync(new(), cancellationToken);
        ...
        private async Task<Run> _WaitForRunAsync(string threadId, string runId, bool handleActions, CancellationToken cancellationToken)
        {
            while (true)
            {
                ...
                Run run = await _client.RetrieveRunAsync(threadId, runId, cancellationToken);
                switch (run.Status)
                {
        ...
        private async Task _BlockOnInProgressRunsAsync(string threadId, CancellationToken cancellationToken)
        {
            ...
            while (true)
            {
                Run? run = await _client.RetrieveLastRunAsync(threadId, cancellationToken);
        ...
        private async Task<Plan> _GeneratePlanFromMessagesAsync(string threadId, string lastMessageId, CancellationToken cancellationToken)
        {
            ...
            IAsyncEnumerable<Message> messages = _client.ListNewMessagesAsync(threadId, lastMessageId, cancellationToken);
        ...
        private async Task<Plan> _SubmitActionResultsAsync(TState state, CancellationToken cancellationToken)
        {
            ...
            Run run = await _client.SubmitToolOutputsAsync(state.ThreadId!, state.RunId!, new()
            {
        ...
        private async Task<Plan> _SubmitUserInputAsync(TState state, CancellationToken cancellationToken)
        {
            ...
            Message message = await _client.CreateMessageAsync(threadId, new()
            {
                Content = state.Temp?.Input ?? string.Empty
            }, cancellationToken);

            // Create a new run
            Run run = await _client.CreateRunAsync(threadId, new()
            {
