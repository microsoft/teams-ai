...
from teams.ai.planners import Planner, Plan, PredictedSayCommand, PredictedDoCommand
...
class LangChainPlanner(Planner[AppTurnState]):
    ...
    async def begin_task(self, context: TurnContext, state: AppTurnState) -> Plan:
        ...
        if len(state.conversation.history) == 0:
            systemPrompt = await template.prompt.render_as_text(
                context,
                state,
                self.prompts,
                self.tokenizer,
            ...
            state.conversation.history.append(Message(role="system", content=systemPrompt.output))
    ...
    async def continue_task(self, context: TurnContext, state: AppTurnState) -> Plan:
        ...
        if last_message.role == "assistant" and last_message.tool_calls is not None:
            for call in last_message.tool_calls:
                ...
                state.conversation.history.append(Message(
                    role="tool",
                    content=output,
    ...
    async def complete_prompt(self, context: TurnContext, state: AppTurnState, template: PromptTemplate) -> Plan:
        self._logger.debug(
            "input => %s",
            state.conversation.history[len(state.conversation.history) - 1]
        )
        ...
        async for chunk in self.model.astream(
            ...
            ] if template.actions is not None else []
        ):
        ...
        for call in res.tool_call_chunks:
            ...
            plan.commands.append(PredictedDoCommand(
                action=name,
                parameters=json.loads(args)
            ))

        if isinstance(res.content, str) and res.content != "":
            plan.commands.append(PredictedSayCommand(
                response=Message(role="assistant", content=res.content)
            ))
