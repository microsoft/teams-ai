...
from teams.streaming.payloads.assemblers import (
    Assembler,
    ReceiveRequestAssembler,
    ReceiveResponseAssembler,
)
from teams.streaming.payloads.models import Header, PayloadTypes
...
class PayloadAssemblerManager:
    ...
    def get_payload_stream(self, header: Header) -> Union[List[int], "streaming.PayloadStream"]:
    ...
    def _create_payload_assembler(self, header: Header) -> Assembler:
        if header.type == PayloadTypes.REQUEST:
            return ReceiveRequestAssembler(header, self._stream_manager, self._on_receive_request)
        if header.type == PayloadTypes.RESPONSE:
            return ReceiveResponseAssembler(header, self._stream_manager, self._on_receive_response)
