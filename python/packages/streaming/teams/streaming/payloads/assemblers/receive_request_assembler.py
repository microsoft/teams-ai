...
import teams.streaming as streaming
import teams.streaming.payloads as payloads
from teams.streaming.payloads.models import Header, RequestPayload
...
class ReceiveRequestAssembler(Assembler):
    ...
    async def process_request(self, stream: List[int]):
        ...
        request = streaming.ReceiveRequest(verb=request_payload.verb, path=request_payload.path, streams=[])
