...
from teams.streaming.transport import TransportReceiverBase, TransportSenderBase
...
class WebSocketTransport(TransportReceiverBase, TransportSenderBase):
    ...
    #  current interfaces
    async def receive(self, buffer: List[int], offset: int = 0, count: int = None) -> int:
        try:
            if self._socket:
                ...
                if result.message_type == WebSocketMessageType.CLOSE:
                    await self._socket.close(WebSocketCloseStatus.NORMAL_CLOSURE, "Socket closed")
