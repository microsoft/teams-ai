...
from teams.streaming.payloads import RequestManager
from teams.streaming.payload_transport import PayloadSender, PayloadReceiver
from teams.streaming.transport import DisconnectedEventArgs
    ...
    async def _on_connection_disconnected(
        ...
        if not self._is_disconnecting:
            ...
            if sender in [self._sender, self._receiver]:
                if iscoroutinefunction(sender.disconnect) or isfuture(sender.disconnect):
