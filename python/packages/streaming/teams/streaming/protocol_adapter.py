...
from teams.streaming.payloads import (
    PayloadAssemblerManager,
    RequestManager,
    SendOperations,
    StreamManager,
)
from teams.streaming.payloads.assemblers import PayloadStreamAssembler
from teams.streaming.payload_transport import PayloadSender, PayloadReceiver
...
class ProtocolAdapter:
    ...
    async def send_request(self, request: StreamingRequest) -> ReceiveResponse:
        if not request:
            raise TypeError(f"'request: {request.__class__.__name__}' argument can't be None")
    ...
    async def _on_receive_request(self, identifier: UUID, request: ReceiveRequest):
        ...
        if self._request_handler:
            response = await self._request_handler.process_request(request, None, self._handler_context)
    ...
    def _on_cancel_stream(self, content_stream_assembler: PayloadStreamAssembler):
        ...
        asyncio.create_task(self._send_operations.send_cancel_stream(content_stream_assembler.identifier))
