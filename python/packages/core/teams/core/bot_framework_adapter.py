...
from teams.connector import Channels, EmulatorApiClient
from teams.connector.aio import ConnectorClient
from teams.connector.auth import (
...
from teams.connector.token_api import TokenApiClient
from teams.connector.token_api.models import (
    TokenStatus,
...
class BotFrameworkAdapterSettings:
    def __init__(
        ...
        :type channel_provider: :class:`teams.connector.auth.ChannelProvider`.  Defaults to SimpleChannelProvider
        if one isn't specified.
        :param auth_configuration:
        :type auth_configuration: :class:`teams.connector.auth.AuthenticationConfiguration`
...
class BotFrameworkAdapter(BotAdapter, ExtendedUserTokenProvider, ConnectorClientBuilder):
    """
    ...
    def __init__(self, settings: BotFrameworkAdapterSettings):
        ...
        if self.settings.open_id_metadata:
            ...
            GovernmentChannelValidation.OPEN_ID_METADATA_ENDPOINT = self.settings.open_id_metadata
    ...
    async def continue_conversation(
        ...
        :type reference: :class:`teams.schema.ConversationReference
        ...
        :type claims_identity: :class:`teams.connector.auth.ClaimsIdentity`
        :param audience:
        ...
        if not reference:
            raise TypeError("Expected reference: ConversationReference but got None instead")
        if not callback:
    ...
    async def create_conversation(
        ...
        :type credentials: :class:`teams.connector.auth.AppCredentials`
    ...
    async def process_activity_with_identity(self, activity: Activity, identity: ClaimsIdentity, logic: Callable):
        ...
        client = await self.create_connector_client(activity.service_url, identity, scope)
        context.turn_state[BotAdapter.BOT_CONNECTOR_CLIENT_KEY] = client
    ...
    async def _authenticate_request(self, request: Activity, auth_header: str) -> ClaimsIdentity:
        """
        Allows for the overriding of authentication in unit tests.

        :param request: The request to authenticate
        :type request: :class:`teams.schema.Activity`
        :param auth_header: The authentication header

        :raises: A permission exception error.

        :return: The request claims identity
        :rtype: :class:`teams.connector.auth.ClaimsIdentity`
        """
    ...
    async def update_activity(self, context: TurnContext, activity: Activity):
        ...
        :type activity: :class:`teams.schema.Activity`
        ...
        .. remarks::
            ...
            a :class:`teams.schema.ResourceResponse` object containing the ID that
    ...
    async def delete_activity(self, context: TurnContext, reference: ConversationReference):
        ...
        .. note::
            ...
            The activity_id of the :class:`teams.schema.ConversationReference` identifies the activity to delete.
        """
        try:
            client = context.turn_state[BotAdapter.BOT_CONNECTOR_CLIENT_KEY]
    ...
    async def delete_conversation_member(self, context: TurnContext, member_id: str) -> None:
        ...
        :param context: The context object for the turn
        :type context: :class:`teams.core.TurnContext`
    ...
    async def get_conversation_members(self, context: TurnContext):
        ...
        client = context.turn_state[BotAdapter.BOT_CONNECTOR_CLIENT_KEY]
        return await client.conversations.get_conversation_members(context.activity.conversation.id)

    async def get_conversation_member(self, context: TurnContext, member_id: str) -> ChannelAccount:
        """
    ...
    async def get_user_token(
        ...
        :rtype: :class:'teams.schema.TokenResponse`
    ...
    async def sign_out_user(
        ...
        client = await self._create_token_api_client(context, oauth_app_credentials)
        client.user_token.sign_out(user_id, connection_name, context.activity.channel_id)
    ...
    async def get_aad_tokens(
        ...
        :returns: Dictionary of resource Urls to the corresponding :class:'teams.schema.TokenResponse`
    ...
    async def get_sign_in_resource_from_user_and_credentials(
        ...
        return client.bot_sign_in.get_sign_in_resource(state, final_redirect=final_redirect)
    ...
    async def exchange_token_from_credentials(
        ...
        if not connection_name:
            raise TypeError("BotFrameworkAdapter.exchange_token(): missing connection_name")
        if not user_id:
    ...
    async def _create_token_api_client(
        ...
        app_credentials = oauth_app_credentials or await self.__get_app_credentials(app_id, scope)
    ...
    async def __get_app_credentials(self, app_id: str, oauth_scope: str) -> AppCredentials:
        if not app_id:
    ...
    async def __build_credentials(self, app_id: str, oauth_scope: str = None) -> AppCredentials:
    ...
    def __get_botframework_oauth_scope(self) -> str:
        if self.settings.channel_provider and self.settings.channel_provider.is_government():
