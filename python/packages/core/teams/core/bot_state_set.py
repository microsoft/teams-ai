...
class BotStateSet:
    ...
    async def load_all(self, turn_context: TurnContext, force: bool = False):
        await wait([bot_state.load(turn_context, force) for bot_state in self.bot_states])

    async def save_all_changes(self, turn_context: TurnContext, force: bool = False):
        await wait([bot_state.save_changes(turn_context, force) for bot_state in self.bot_states])
