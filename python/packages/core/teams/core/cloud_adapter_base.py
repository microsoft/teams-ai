...
from teams.core.invoke_response import InvokeResponse
...
from teams.connector import Channels, ConnectorClient
...
class CloudAdapterBase(BotAdapter, ABC):
    ...
    async def send_activities(self, context: TurnContext, activities: List[Activity]) -> List[ResourceResponse]:
        ...
        for activity in activities:
            ...
            elif activity.type == ActivityTypes.trace and activity.channel_id != Channels.emulator:
                # no-op
                pass
            else:
    ...
    async def process_activity(
        self,
        auth_header_or_authenticate_request_result: Union[str, AuthenticateRequestResult],
        activity: Activity,
        logic: Callable[[TurnContext], Awaitable],
    ):
        """
    ...
    def _process_turn_results(self, context: TurnContext) -> InvokeResponse:
        ...
        if context.activity.type == ActivityTypes.invoke:
            activity_invoke_response: Activity = context.turn_state.get(self._INVOKE_RESPONSE_KEY)
