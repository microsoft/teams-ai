...
from teams.schema import ChannelAccount, ErrorResponseException, SignInConstants
from teams.core import ActivityHandler, InvokeResponse
from teams.core.activity_handler import _InvokeResponseException
from teams.core.turn_context import TurnContext
from teams.core.teams.teams_info import TeamsInfo
...
from teams.connector import Channels
...
class TeamsActivityHandler(ActivityHandler):
        ...
        try:
            ...
            if turn_context.activity.name == "fileConsent/invoke":
                return await self.on_teams_file_consent(
                    turn_context,
                    deserializer_helper(FileConsentCardResponse, turn_context.activity.value),
                )

            if turn_context.activity.name == "actionableMessage/executeAction":
                await self.on_teams_o365_connector_card_action(
                    turn_context,
                    deserializer_helper(O365ConnectorCardActionQuery, turn_context.activity.value),
                )
                return self._create_invoke_response()
            ...
            if turn_context.activity.name == "composeExtension/queryLink":
                return self._create_invoke_response(
                    await self.on_teams_app_based_link_query(
                        turn_context,
                        deserializer_helper(AppBasedLinkQuery, turn_context.activity.value),
                    )
                )
            ...
            if turn_context.activity.name == "composeExtension/query":
                return self._create_invoke_response(
                    await self.on_teams_messaging_extension_query(
                        turn_context,
                        deserializer_helper(MessagingExtensionQuery, turn_context.activity.value),
                    )
                )
            ...
            if turn_context.activity.name == "composeExtension/submitAction":
                return self._create_invoke_response(
                    await self.on_teams_messaging_extension_submit_action_dispatch(
                        turn_context,
                        deserializer_helper(MessagingExtensionAction, turn_context.activity.value),
                    )
                )
            ...
            if turn_context.activity.name == "task/fetch":
                return self._create_invoke_response(
                    await self.on_teams_task_module_fetch(
                        turn_context,
                        deserializer_helper(TaskModuleRequest, turn_context.activity.value),
                    )
                )
    ...
    async def on_teams_card_action_invoke(self, turn_context: TurnContext) -> InvokeResponse:
        """
    ...
    async def on_teams_file_consent(
        ...
        if file_consent_card_response.action == "decline":
            await self.on_teams_file_consent_decline(turn_context, file_consent_card_response)
            return self._create_invoke_response()
    ...
    async def on_conversation_update_activity(self, turn_context: TurnContext):
        ...
        if turn_context.activity.channel_id == Channels.ms_teams:
            channel_data = TeamsChannelData().deserialize(turn_context.activity.channel_data)
            ...
            if channel_data:
                ...
                if channel_data.event_type == "teamRestored":
                    return await self.on_teams_team_restored(channel_data.team, turn_context)
                if channel_data.event_type == "teamUnarchived":
                    return await self.on_teams_team_unarchived(channel_data.team, turn_context)
    ...
    async def on_teams_members_added_dispatch(  # pylint: disable=unused-argument
        ...
        return await self.on_teams_members_added(team_members_added, team_info, turn_context)
    ...
    async def on_teams_members_removed_dispatch(  # pylint: disable=unused-argument
        ...
        for member in members_removed:
            ...
            teams_members_removed.append(TeamsChannelAccount().deserialize(new_account_json))
