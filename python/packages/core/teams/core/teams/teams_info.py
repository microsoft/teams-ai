...
class TeamsInfo:
    ...
    @staticmethod
    async def get_team_details(turn_context: TurnContext, team_id: str = "") -> TeamDetails:
        if not team_id:
    ...
    @staticmethod
    async def get_team_channels(turn_context: TurnContext, team_id: str = "") -> List[ChannelInfo]:
        if not team_id:
            team_id = TeamsInfo.get_team_id(turn_context)
    ...
    async def get_member(turn_context: TurnContext, member_id: str) -> TeamsChannelAccount:
        ...
        if not team_id:
            conversation_id = turn_context.activity.conversation.id
            connector_client = await TeamsInfo._get_connector_client(turn_context)
            return await TeamsInfo._get_member(connector_client, conversation_id, member_id)
    ...
    async def get_meeting_participant(
        ...
        meeting_id = meeting_id if meeting_id else teams_get_meeting_info(turn_context.activity).id
    ...
    async def _get_paged_members(
        ...
        if connector_client is None:
            raise TypeError("TeamsInfo._get_paged_members.connector_client: cannot be None.")

        if not conversation_id:
            raise TypeError("TeamsInfo._get_paged_members.conversation_id: cannot be empty.")
