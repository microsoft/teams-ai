...
from teams.connector.teams.teams_connector_client import TeamsConnectorClient
from teams.schema import ConversationParameters, ConversationReference
from teams.core.teams.teams_activity_extensions import (
    teams_get_meeting_info,
    teams_get_channel_data,
)
from teams.core.turn_context import Activity, TurnContext
from teams.schema.teams import (
    ChannelInfo,
    MeetingInfo,
    TeamDetails,
    TeamsChannelData,
...
class TeamsInfo:
    ...
    @staticmethod
    async def get_team_details(turn_context: TurnContext,
                               team_id: str = "") -> TeamDetails:
        if not team_id:
        ...
        teams_connector = await TeamsInfo.get_teams_connector_client(
            turn_context)
        return teams_connector.teams.get_team_details(team_id)

    @staticmethod
    async def get_team_channels(turn_context: TurnContext,
                                team_id: str = "") -> List[ChannelInfo]:
        if not team_id:
            team_id = TeamsInfo.get_team_id(turn_context)
    ...
    @staticmethod
    async def get_team_members(turn_context: TurnContext,
                               team_id: str = "") -> List[TeamsChannelAccount]:
        if not team_id:
            team_id = TeamsInfo.get_team_id(turn_context)
    ...
    async def get_members(
            turn_context: TurnContext) -> List[TeamsChannelAccount]:
        team_id = TeamsInfo.get_team_id(turn_context)
        if not team_id:
            conversation_id = turn_context.activity.conversation.id
            connector_client = await TeamsInfo._get_connector_client(
                turn_context)
            return await TeamsInfo._get_members(connector_client,
                                                conversation_id)
    ...
    async def get_paged_members(
            turn_context: TurnContext,
            continuation_token: str = None,
            page_size: int = None) -> List[TeamsPagedMembersResult]:
        ...
        if not team_id:
            ...
            connector_client = await TeamsInfo._get_connector_client(
                turn_context)
            return await TeamsInfo._get_paged_members(connector_client,
                                                      conversation_id,
                                                      continuation_token,
                                                      page_size)
    ...
    @staticmethod
    async def get_team_member(turn_context: TurnContext,
                              team_id: str = "",
        ...
        if not member_id:
            raise TypeError(
                "TeamsInfo.get_team_member: method requires a member_id")
    ...
    @staticmethod
    async def get_member(turn_context: TurnContext,
        ...
        if not team_id:
            ...
            connector_client = await TeamsInfo._get_connector_client(
                turn_context)
            return await TeamsInfo._get_member(connector_client,
    ...
    async def get_meeting_participant(
        ...
        meeting_id = (meeting_id if meeting_id else teams_get_meeting_info(
        ...
        tenant_id = (tenant_id if tenant_id else teams_get_channel_data(
            turn_context.activity).tenant.id)
        if tenant_id is None:
            raise TypeError(
                "TeamsInfo._get_meeting_participant: method requires a tenant_id"
            )

        connector_client = await TeamsInfo.get_teams_connector_client(
            turn_context)
        return connector_client.teams.fetch_participant(
            meeting_id, participant_id, tenant_id)

    @staticmethod
    async def get_meeting_info(turn_context: TurnContext,
                               meeting_id: str = None) -> MeetingInfo:
        meeting_id = (meeting_id if meeting_id else teams_get_meeting_info(
            turn_context.activity).id)
        if meeting_id is None:
            raise TypeError(
    ...
    async def get_teams_connector_client(
        turn_context: TurnContext, ) -> TeamsConnectorClient:
    ...
    @staticmethod
    async def _get_connector_client(
            turn_context: TurnContext) -> ConnectorClient:
    ...
    async def _get_paged_members(
        ...
        return (await connector_client.conversations.
                get_teams_conversation_paged_members(conversation_id,
                                                     page_size,
                                                     continuation_token))
