...
class MemoryStorage(Storage):
    ...
    async def write(self, changes: Dict[str, StoreItem]):
        ...
        try:
            ...
            for (key, change) in changes.items():
                ...
                if (old_state_etag is not None and new_value_etag is not None
                        and new_value_etag != "*"
    ...
    def __should_write_changes(self, old_value: StoreItem,
                               new_value: StoreItem) -> bool:
        """
        ...
        if old_value is None or (hasattr(new_value, "e_tag")
                                 and new_value.e_tag == "*"):
            return True
        ...
        if hasattr(new_value, "e_tag") and hasattr(old_value, "e_tag"):
            ...
            if old_value.e_tag == new_value.e_tag or int(
                    old_value.e_tag) <= int(new_value.e_tag):
                return True
            return False
        return False
