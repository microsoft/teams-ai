...
from teams.core import Middleware, TurnContext
...
class InterceptionMiddleware(Middleware):
    async def on_turn(self, context: TurnContext, logic: Callable[[TurnContext], Awaitable]):
        ...
        if should_intercept:
            ...
            async def aux_on_update(ctx: TurnContext, activity: Activity, next_update: Callable):
                trace_activity = from_activity(activity, "MessageUpdate", "Updated Message")
    ...
    @abstractmethod
    async def _outbound(self, context: TurnContext, trace_activities: List[Activity]) -> Any:
        raise NotImplementedError()
    ...
    async def _invoke_inbound(self, context: TurnContext, trace_activity: Activity) -> Any:
        try:
    ...
    async def _invoke_outbound(self, context: TurnContext, trace_activities: List[Activity]) -> Any:
        try:
