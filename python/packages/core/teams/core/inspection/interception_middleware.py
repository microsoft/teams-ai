...
class InterceptionMiddleware(Middleware):
    ...
    async def on_turn(self, context: TurnContext,
        ...
        should_forward_to_application, should_intercept = await self._invoke_inbound(
            context,
            from_activity(context.activity, "ReceivedActivity",
                          "Received Activity"),
        )
        ...
        if should_intercept:
            ...
            async def aux_on_update(ctx: TurnContext, activity: Activity,
                ...
                trace_activity = from_activity(activity, "MessageUpdate",
                                               "Updated Message")
    ...
    async def _invoke_inbound(self, context: TurnContext,
                              trace_activity: Activity) -> Any:
        try:
    ...
    async def _invoke_outbound(self, context: TurnContext,
                               trace_activities: List[Activity]) -> Any:
        try:
