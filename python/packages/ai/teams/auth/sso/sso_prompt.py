...
from botbuilder.schema import TokenResponse, OAuthCard, ActivityTypes, SignInConstants, Activity, TokenExchangeInvokeResponse, InvokeResponse
...
class SsoPrompt(Dialog):
    ...
    def __init__(self, dialog_id: str, name: str, options: SsoOptions, msal: ConfidentialClientApplication) -> None:
    ...
    async def begin_dialog(self, dialog_context: "DialogContext", options: object = None):
        ...
        if self._options.timeout:
            if self._options.timeout <= 0:
                raise Exception("value of timeout property in teamsBotSsoPromptSettings should be positive.")
        ...
        if token:
            token_response = TokenResponse(connection_name="", token=token["access_token"], expiration=cast(datetime,token["expires_on"]).isoformat())
    ...
    async def acquire_token_from_cache(self, context: TurnContext) -> Optional[dict[str, Union[str, Any]]]:
        ...
        if aad_object_id:
            ...
            for account in accounts:
                if account["local_account_id"] == aad_object_id:
                    return self._msal.acquire_token_silent(scopes=self._options.scopes, account=account)
    ...
    async def continue_dialog(self, dialog_context: "DialogContext"):
        ...
        if state:
            has_timed_out = is_timeout_activity_type and datetime.now() > cast(datetime, state["expires"])
    ...
    async def _recognize_token(self, dialog_context: DialogContext) -> PromptRecognizerResult:
        ...
        if self._is_exchange_activity(context):
            ...
            if not (context.activity.value and self._is_exchange_activity(context.activity.value)):
                warning_message = "The bot received an InvokeActivity that is missing a TokenExchangeInvokeRequest value. This is required to be sent with the InvokeActivity."
                await context.send_activity(Activity(type=ActivityTypes.invoke_response, value=TokenExchangeInvokeResponse(status=400, failure_detail=warning_message)))
            else:
                sso_token = context.activity.value.get("token")

                exchanged_token = self._msal.acquire_token_on_behalf_of(user_assertion=sso_token, scopes=self._options.scopes)

                if exchanged_token:
                    await context.send_activity(Activity(type=ActivityTypes.invoke_response, value=TokenExchangeInvokeResponse(status=200, id=context.activity.value.get("id"))))
                    ...
                    token_response = TokenResponse(token=access_token, expiration=expiry_date.isoformat())

        if self._is_verify_state_activity(context):
            await self.send_oauth_card_async(context)
            await context.send_activity(Activity(type=ActivityTypes.invoke_response, value=InvokeResponse(status=200)))
    ...
    def _get_sign_in_resource(self):
        ...
        sign_in_link = f"{self._options.sign_in_link}?scope={scope}&clientId={client_id}&tenantId={tenant_id}"
