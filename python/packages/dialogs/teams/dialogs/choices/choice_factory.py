...
from teams.core import CardFactory, MessageFactory
from teams.schema import ActionTypes, Activity, CardAction, HeroCard, InputHints
...
class ChoiceFactory:
    ...
    def inline(
        ...
        opt = ChoiceFactoryOptions(
            ...
            include_numbers=(options.include_numbers
                             if options.include_numbers is not None else True),
        )
        ...
        for index, choice in enumerate(choices):
            title = (choice.action.title if
                     (choice.action is not None
        ...
        return MessageFactory.text("".join(txt_builder), speak,
    ...
    @staticmethod
    def hero_card(choices: List[Union[Choice, str]],
                  text: str = None,
                  speak: str = None) -> Activity:
        """
    ...
    def _extract_actions(
            choices: List[Union[str, Choice]]) -> List[CardAction]:
        if choices is None:
            choices = []
        choices = ChoiceFactory._to_choices(choices)
        ...
        for choice in choices:
            if choice.action is not None:
                ...
                card_action = CardAction(type=ActionTypes.im_back,
                                         value=choice.value,
                                         title=choice.value)
