...
class DialogClassMemoryScope(MemoryScope):
    ...
    def __init__(self):
        ...
        from teams.dialogs import DialogContainer
    ...
    def get_memory(self, dialog_context: "DialogContext") -> object:
        ...
        parent_id = (dialog_context.parent.active_dialog.id
                     if dialog_context.parent
                     and dialog_context.parent.active_dialog else None)
        active_id = (dialog_context.active_dialog.id
                     if dialog_context.active_dialog else None)
        return deepcopy(dialog_context.find_dialog_sync(parent_id
                                                        or active_id))
