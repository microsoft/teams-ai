...
from teams.core.skills import SkillConversationReference, SkillHandler
from teams.dialogs.memory import DialogStateManagerConfiguration
from teams.schema import Activity, ActivityTypes, EndOfConversationCodes
...
class DialogManager:
    ...
    def __init__(self,
                 root_dialog: Dialog = None,
                 dialog_state_property: str = None):
        """
    ...
    async def on_turn(self, context: TurnContext) -> DialogManagerResult:
        ...
        if self.conversation_state is None:
            ...
            context.turn_state[
                conversation_state_name] = self.conversation_state
        ...
        last_access_property = self.conversation_state.create_property(
            self.last_access)
        ...
        if self.expire_after is not None and (
                datetime.now() - last_access) >= timedelta(
                    milliseconds=float(self.expire_after)):
    ...
    def should_send_end_of_conversation_to_parent(
            context: TurnContext, turn_result: DialogTurnResult) -> bool:
        """
        ...
        :return:
        """
        if not (turn_result.status == DialogTurnStatus.Complete
    ...
    async def handle_skill_on_turn(
        ...
        if (turn_context.activity.type == ActivityTypes.end_of_conversation
            ...
            active_dialog_context = self.get_active_dialog_context(
                dialog_context)
        ...
        if (turn_context.activity.type == ActivityTypes.event and
                turn_context.activity.name == DialogEvents.reprompt_dialog):
        ...
        if turn_result.status == DialogTurnStatus.Empty:
            # restart root dialog
            turn_result = await dialog_context.begin_dialog(
                self._root_dialog_id)

        await DialogManager.send_state_snapshot_trace(dialog_context,
                                                      "Skill State")

        if self.should_send_end_of_conversation_to_parent(
                turn_context, turn_result):
            ...
            activity = Activity(
                ...
                if turn_result.status == DialogTurnStatus.Complete else
