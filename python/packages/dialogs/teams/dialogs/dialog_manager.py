...
class DialogManager:
    ...
    async def on_turn(self, context: TurnContext) -> DialogManagerResult:
        ...
        if self.expire_after is not None and (datetime.now() - last_access) >= timedelta(
    ...
    async def handle_skill_on_turn(self, dialog_context: DialogContext) -> DialogTurnResult:
        warn(
            "This method will be deprecated as no longer is necesary",
            PendingDeprecationWarning,
        )
    ...
    async def handle_bot_on_turn(self, dialog_context: DialogContext) -> DialogTurnResult:
        warn(
            "This method will be deprecated as no longer is necesary",
            PendingDeprecationWarning,
        )
