...
from teams.core.bot_framework_adapter import TokenExchangeRequest
from teams.core.oauth import ConnectorClientBuilder, ExtendedUserTokenProvider
from teams.schema import TokenResponse
from teams.connector import ConnectorClient
from teams.connector.auth import ClaimsIdentity, ConnectorFactory
from teams.connector.auth.user_token_client import UserTokenClient
from teams.connector.token_api.models import SignInUrlResponse
...
class _UserTokenAccess(ABC):
    ...
    @staticmethod
    async def get_user_token(turn_context: TurnContext,
    ...
    async def get_sign_in_resource(
            ...
            settings: OAuthPromptSettings) -> SignInUrlResponse:
        user_token_client: UserTokenClient = turn_context.turn_state.get(
            UserTokenClient.__name__, None)
        if user_token_client:
        ...
        if isinstance(turn_context.adapter, ExtendedUserTokenProvider):
            return await turn_context.adapter.get_sign_in_resource_from_user_and_credentials(
                ...
                settings.oath_app_credentials,
                settings.connection_name,
                turn_context.activity.from_property.id if turn_context.activity
                and turn_context.activity.from_property else None,
            )

        raise TypeError("OAuthPrompt is not supported by the current adapter")

    @staticmethod
    async def sign_out_user(turn_context: TurnContext,
                            settings: OAuthPromptSettings):
        user_token_client: UserTokenClient = turn_context.turn_state.get(
            UserTokenClient.__name__, None)
        if user_token_client:
