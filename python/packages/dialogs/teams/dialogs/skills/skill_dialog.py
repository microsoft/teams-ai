...
from teams.connector.token_api.models import TokenExchangeRequest
...
from teams.core import BotAdapter, TurnContext, ExtendedUserTokenProvider
from teams.core.card_factory import ContentTypes
from teams.core.skills import SkillConversationIdFactoryOptions
from teams.dialogs import (
    Dialog,
    DialogContext,
    DialogEvents,
    DialogReason,
...
class SkillDialog(Dialog):
    ...
    def __init__(self, dialog_options: SkillDialogOptions, dialog_id: str):
        ...
        if not dialog_options:
            raise TypeError(
                "SkillDialog.__init__(): dialog_options cannot be None.")
    ...
    async def begin_dialog(self,
        ...
        dialog_context.active_dialog.state[
            self._deliver_mode_state_key] = dialog_args.activity.delivery_mode
        ...
        dialog_context.active_dialog.state[
            SkillDialog.SKILLCONVERSATIONIDSTATEKEY] = skill_conversation_id

        # Send the activity to the skill.
        eoc_activity = await self._send_to_skill(dialog_context.context,
                                                 skill_activity,
    ...
    def _validate_begin_dialog_args(
            self, options: object) -> BeginSkillDialogOptions:
        if not options:
    ...
    async def _send_to_skill(self, context: TurnContext, activity: Activity,
                             skill_conversation_id: str) -> Activity:
            ...
            for from_skill_activity in response.body:
                if from_skill_activity.type == ActivityTypes.end_of_conversation:
                    ...
                    await self.dialog_options.conversation_id_factory.delete_conversation_reference(
                        ...
                        context, from_skill_activity,
                        self.dialog_options.connection_name):
    ...
    async def _create_skill_conversation_id(self, context: TurnContext,
    ...
    async def _intercept_oauth_cards(self, context: TurnContext,
                                     activity: Activity, connection_name: str):
        """
        Tells is if we should intercept the OAuthCard message.
        """
        if not connection_name or not isinstance(context.adapter,
        ...
        if oauth_card_attachment:
            ...
            if (oauth_card and oauth_card.token_exchange_resource
