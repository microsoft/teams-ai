...
class SkillDialog(Dialog):
    SKILLCONVERSATIONIDSTATEKEY = "Microsoft.Bot.Builder.Dialogs.SkillDialog.SkillConversationId"
    ...
    async def continue_dialog(self, dialog_context: DialogContext):
        ...
        if dialog_context.context.activity.type == ActivityTypes.end_of_conversation:
            return await dialog_context.end_dialog(dialog_context.context.activity.value)
    ...
    def _on_validate_activity(self, activity: Activity) -> bool:  # pylint: disable=unused-argument
        """
    ...
    async def _create_skill_conversation_id(self, context: TurnContext, activity: Activity) -> str:
    ...
    async def _intercept_oauth_cards(self, context: TurnContext, activity: Activity, connection_name: str):
        ...
        """
        if not connection_name or not isinstance(context.adapter, ExtendedUserTokenProvider):
