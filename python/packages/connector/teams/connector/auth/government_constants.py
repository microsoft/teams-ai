# Copyright (c) Microsoft Corporation. All rights reserved.
# Licensed under the MIT License.

from abc import ABC


class GovernmentConstants(ABC):
    """
    Government Channel Service property value
    """

    CHANNEL_SERVICE = "https://teams.azure.us"
    """
    TO CHANNEL FROM BOT: Login URL
    """
    TO_CHANNEL_FROM_BOT_LOGIN_URL = "https://login.microsoftonline.us/MicrosoftServices.onmicrosoft.us"
    """
    TO CHANNEL FROM BOT: OAuth scope to request
    """
    TO_CHANNEL_FROM_BOT_OAUTH_SCOPE = "https://api.teams.us/.default"
    """
    TO BOT FROM CHANNEL: Token issuer
    """
    TO_BOT_FROM_CHANNEL_TOKEN_ISSUER = "https://api.teams.us"
    """
    OAuth Url used to get a token from OAuthApiClient.
    """
    OAUTH_URL_GOV = "https://api.teams.azure.us"
    """
    TO BOT FROM CHANNEL: OpenID metadata document for tokens coming from MSA
    """
    TO_BOT_FROM_CHANNEL_OPEN_ID_METADATA_URL = "https://login.teams.azure.us/v1/.well-known/openidconfiguration"
    """
    TO BOT FROM GOV EMULATOR: OpenID metadata document for tokens coming from MSA
    """
    TO_BOT_FROM_EMULATOR_OPEN_ID_METADATA_URL = (
        "https://login.microsoftonline.us/"
        "cab8a31a-1906-4287-a0d8-4eef66b95f6e/v2.0/"
        ".well-known/openid-configuration"
    )
