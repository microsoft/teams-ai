...
from teams.schema import Activity, RoleTypes
...
class _ParameterizedBotFrameworkAuthentication(BotFrameworkAuthentication):
    ...
    async def _jwt_token_validation_authenticate_request(
            self, activity: Activity, auth_header: str) -> ClaimsIdentity:
    ...
    async def _jwt_token_validation_validate_auth_header(
            ...
            auth_header: str,
            channel_id: str,
            service_url: Optional[str] = None) -> ClaimsIdentity:
    ...
    async def _jwt_token_validation_validate_claims(self,
    ...
    async def _skill_validation_authenticate_channel_token(
            self, auth_header: str,
            channel_id: str) -> Optional[ClaimsIdentity]:
        if not auth_header:
            return None
        ...
        token_extractor = JwtTokenExtractor(
            ...
            allowed_algorithms=AuthenticationConstants.
        ...
        identity = await token_extractor.get_identity(
            ...
            required_endorsements=self._auth_configuration.
    ...
    async def _skill_validation_validate_identity(self,
    ...
    async def _emulator_validation_authenticate_emulator_token(
        ...
        if not version_claim or version_claim == "1.0":
            ...
            app_id = identity.get_claim_value(
            ...
            raise PermissionError(
                f"Unknown Emulator Token version '{version_claim}'.")
    ...
    async def _government_channel_validation_authenticate_channel_token(
            self, auth_header: str, service_url: str,
            channel_id: str) -> Optional[ClaimsIdentity]:
        if not auth_header:
            return None
        ...
        validation_params = VerifyOptions(
            ...
            audience=
            None,    # Audience validation takes place in JwtTokenExtractor
            clock_tolerance=5 * 60,
            ignore_expiration=False,
        )
