...
from teams.schema import Activity, ConversationReference, ChannelAccount, RoleTypes
...
class TestAuth:
    ...
    async def test_government_channel_validation_succeeds(self):
        ...
        await GovernmentChannelValidation.validate_identity(
            ClaimsIdentity(
                {"iss": "https://api.teams.us", "aud": credentials.app_id}, True
            ),
            credentials,
        )
    ...
    #        with pytest.raises(Exception) as excinfo:
    #            await GovernmentChannelValidation.validate_identity(
    #                ClaimsIdentity({"iss": "https://api.teams.us"}, True),
    #                credentials,
    #            )
    #        assert "Unauthorized" in str(excinfo.value)
    ...
    async def test_government_channel_validation_wrong_audience_fails(self):
        ...
        with pytest.raises(Exception) as excinfo:
            await GovernmentChannelValidation.validate_identity(
                ClaimsIdentity(
                    {"iss": "https://api.teams.us", "aud": "peanut"}, True
                ),
                credentials,
            )
        assert "Unauthorized" in str(excinfo.value)
